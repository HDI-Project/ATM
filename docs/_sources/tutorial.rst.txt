...
Data Format
-----------
BTB takes data in CSV format where each CSV file needs to:
...
To run BTB, you must create a configuration file.
A configuration file template is included in ``config/btb.cnf.template`` (and shown below).
Since the configuration file contains passwords, it's best to rename it to ``btb.cnf`` so that it will be ignored by git.
This is especially true if you plan to make changes to BTB and upload them to the repository.
The git repository is setup to ignore all files in the ``config`` folder except ``btb.cnf.template``.

.. literalinclude:: ../config/btb.cnf.template

The name of the file must also be a environmental variable called ``BTB_CONFIG_FILE``.
For example if the configuration file is called ``btb.cnf`` in the ``config`` directory of the root btb directory, then an environmental variable would created with the command::

    (btb-env) $ export BTB_CONFIG_FILE=/path_to_btb_root/config/btb.cnf
...
The datarun ID in the database also ties together the `hyperpartitions` (frozen sets) which delineate how BTB can explore different subtypes of classifiers to maximize their performance.
Once the configuration file is filled out, we can enter it in DataHub with::

    (btb-env) $ python enter_data.py
...
First, you must create instance(s) by executing the following command on your local machine::
    ...
    (btb-env) $ fab create_instances

The number of instances will be the number specified in the *aws* section of the configuration file.
Second, you must start BTB worker processes on the instance(s) by executing the following command on your local machine::
