...
from atm.utilities import obj_has_method
...
class Predictor(object):
    ...
    def __init__(self, models, ensemble_type='voting'):
        ...
        if self.ensemble_type == Predictor.ENSEMBLE_STACKING:
            ...
            for model in self.models:
                ...
                if obj_has_method(clf, "predict_proba"):
    ...
    def setup(self, trainingX=None, trainingY=None, input_type=Model.INPUT_CSV,
              stacker=None):
        if self.ensemble_type == Predictor.ENSEMBLE_STACKING:
            ...
            if stacker:
                ...
                for j, model in enumerate(self.models):
                    output = model.predict(trainingX, input_type=input_type,
                                           probability=True)
                ...
                for j, model in enumerate(self.models):
                    estimates[:, j] = model.predict(trainingX,
                                                    input_type=input_type,
                                                    probability=True)

                # fit the outputs of the ensemble classifiers to the original labels,
                # forming a model of these outputs => original labeling
                #print "Stacker (%d models) is fitting %s to %s" %\
                #   (len(self.models), estimates, trainingY)
                self.stacker.fit(estimates, trainingY)

    def predict(self, examples, input_type=Model.INPUT_CSV,
                output_type=Model.OUTPUT_ORIGINAL):
        return self.prediction_map[self.ensemble_type](examples, input_type,
                                                       output_type)

    def vote(self, examples, input_type=Model.INPUT_CSV, output_type=Model.OUTPUT_ORIGINAL):
        """
            Takes all of the models in this predictor and uses them to vote on
            the correct labels.  Voting method is plurality.
        """
        ...
        for j, model in enumerate(self.models):
            votes[:, j] = model.predict(examples, input_type=input_type,
                                        output_type=Model.OUTPUT_INT).astype(int)
    ...
    def stack(self, examples, input_type=Model.INPUT_CSV,
              output_type=Model.OUTPUT_ORIGINAL):
        """
            Uses a probibabilistic ensemble to generate an output vector for
            each original training example based on the prediction from each of
            the classifiers in the ensemble.

            The stacker, pretrained in setup uses the output signals to predict
            the correct label.
        """
        n, m = len(examples), len(self.models)
        estimates = np.zeros((n, m))
        ...
        # get each model's probabilistic input
        for j, model in enumerate(self.models):
            estimates[:, j] = model.predict(examples, input_type=input_type,
                                            probability=True)
